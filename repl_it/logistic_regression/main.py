...
def plotData(X, y):
    # plots the data points with o for the positive examples and x for the
    # negative examples. output is saved to file graph.png
    fig, ax = plt.subplots(figsize=(12, 8))
    # TODO insert your code here

    positive = y > 0
    negative = y < 0
    ax.scatter(X[positive, 0],
               X[positive, 1],
               c='b',
               marker='o',
               label='Healthy')
    ax.scatter(X[negative, 0],
               X[negative, 1],
               c='r',
               marker='x',
               label='Not Healthy')
...
def computeCost(X, y, theta):
    ...
    m, n = X.shape
    cost = np.sum(
        np.log(1 + np.exp(np.multiply(-y, np.dot(X, np.array(theta))))))
    return cost * 1.0 / m
...
def computeGradient(X, y, theta):
    ...
    for j in range(n):
        exps = np.exp(-y * np.dot(X, np.array(theta)))
        sum = np.sum(-y * X[:, j] * exps / (1 + exps))
        grad[j] = sum * 1.0 / m
...
def computeScore(X, y, preds):
    # for training data X,y it calculates the number of correct predictions
    # made by the model
    # TODO replace the next line with your code
    n = len(preds)
    score = 0
    for i in range(n):
        if np.sign(y[i]) == np.sign(preds[i]):
            score += 1
    return score
...
def main():
    ...
    print('approx expected value is [1.11,2.42,2.29] for a linear boundary, ' +
          'and [3.08,2.97,3.69,-5.36] when using a quadratic boundary' +
    ...
    print('approx expected value is 68%, increasing to 78% ' +
          'when quadratic boundary is used')
