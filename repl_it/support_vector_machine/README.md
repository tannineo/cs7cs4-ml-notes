...
$$
J(\theta) = \frac{1}{m} \sum^m_{i = 1} \max(0, 1 - y^{(i)}\theta^Tx^{(i)}) + \lambda\theta^T\theta
$$

and the gradient of the cost is a vector where the jth element is:

$$
2\lambda\theta_j - \frac{1}{m} \sum^m_{i = 1} y^{(i)} x_j^{(i)} 1(y^{(i)}\theta^Tx^{(i)} \le 1)
$$

### Implementing an SVM

As usual, your first task is to complete the code in functions `predict()`, `computeCost()` and `computeGradient()` that implement $h_\theta(x)$, $J(\theta)$ and `dJ/dθ_j` respectively. Make sure that these functions work for any number of features and data points.

Try to avoid using for loops since these are inefficient in python. And remember to use numpy arrays rather than numpy matrices.
...
The cost $J(\theta)$ depends on parameter `𝜆`, and so the value of `𝜆` affects the values of θ that minimise $J(\theta)$. Try some values for `𝜆` and see how they affect the value of $\theta$ obtained after running gradient descent e.g. try values of `0, 0.01, 0.1, 1, 10`. Keep an eye on the plot in `cost.png` to make sure that the gradient descent is converging, and adjust `α` if necessary.
...
$$
K(x, z) = e^{\frac{\sum^n_{j = 1}(x_j - z_j)^2}{\sigma^2}}
$$
...
![sigma^2 = 1](https://storage.googleapis.com/replit/images/1506524421369_f06b56273e4062fd69da17b2dd6f0fa8.png)
...
![pred2.png](https://storage.googleapis.com/replit/images/1506526302468_3c251f11cbb838e324170c0fcefbe790.png)

and the accuracy should be reported as around 78% (similar to when a quadratic boundary was used).

### Selecting C and gamma (𝜆 and σ) using cross-validation
...
A common approach is to loop through a range of values for C and gamma and for each value we estimate the `predictive` accuracy using cross-validation. We select the values of C and gamma which give the best predictive accuracy.

The predictive accuracy is the accuracy of our model when presented with new data (i.e. different from the training data). We can estimate this using cross-validation. That is, by randomly splitting the data that we have into a part used for training the model and a part used for testing the model, training the model and measuring its accuracy and then repeating this several times (i.e. for several different partitions of the data).
...
1. scans over a range of values of C and gamma
2. calls `cross_val_score()` to estimate the prediction accuracy for each pair of values
3. plots this data (a 3D plot is needed since we have two values C and gamma plus the corresponding accuracy score.
...
![plot](https://storage.googleapis.com/replit/images/1506528223400_ffc95d527add7adcc13580fe6551a249.png)
...
## Optional Exercise: Sentiment analysis (ungraded)
...
The full data set is available at http://www.cs.cornell.edu/people/pabo/movie-review-data/

but we'll use a subset consisting of 800 reviews that is contained in file polar_data.json. This data (review text plus label) has been transformed to json format to make it easier to work with.
...
which holds 10% of the data back for testing and uses 90% for training. See:

http://scikit-learn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html

### Mapping the review text to a feature vector
...
For each review we now construct a vector of length $|D|$ which has element $x_i$ set equal to the number of times the i'th word in dictionary D appears in the review. For example, suppose `D=["terrible", "mess", "man", "action"]` and the review text is "a terrible mess of a movie starring a terrible mess of a man".

Then the vector associated with the review is `[2, 2, 1, 0]` since the word "terrible" occurs twice, the word "mess" occurs twice, the work "man" appears once and so on. We do not include common words such as "a" and "of" in the dictionary and so these do not contribute to the vector.
