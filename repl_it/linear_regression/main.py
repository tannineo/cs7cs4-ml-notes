...
def predict(X, theta):
    # takes m by n matrix X as input and returns an mx1 vector containing the
    # predictions h_theta(x^i) for each row x^i, i=1,...,m in X
    # TODO replace the next line with your code
    pred = np.dot(X, np.array([theta[0], theta[1]]))
    return pred
...
def computeCost(X, y, theta):
    ...
    # TODO replace the next line with your code
    (m, n) = X.shape
    cost = np.multiply(1.0 / 2.0 / m,
                       np.sum(np.power(predict(X, theta) - y, 2.0)))
    return cost
...
def computeGradient(X, y, theta):
    ...
    grad = np.multiply(1.0 / m, np.dot(pred - y, X))
    return grad
...
def main():
    ...
    theta = (1, 2)
    print('when x=[1,1] and theta is [1,2]) cost = ',
          predict(np.ones(n), theta))
    print('approx expected prediction is 3')
    print('when x=[[1,1],[5,5]] and theta is [1,2]) cost = ',
          predict(np.array([[1, 1], [5, 5]]), theta))
